...
bool	parse_direction(char *str, t_vector *direction)
{
	...
	if (!validate_count(splitted, 3)
		...
		|| !validate_direction_range(splitted[0], &direction->x)
		|| !parse_double(splitted[1], &direction->y)
		|| !validate_direction_range(splitted[1], &direction->y)
		|| !parse_double(splitted[2], &direction->z)
		|| !validate_direction_range(splitted[2], &direction->z)
		// || fabs(1 - magnitude(*direction)) >= EPSILON	// Não sei se precisa exigir que
		)													// o vetor seja normalizado
	{														// Essa linha comentada, tira isso
		ft_free_split(splitted);							// Mas precisa verificar se a direção
		return (false);										// está entre -1 e 1
	}
